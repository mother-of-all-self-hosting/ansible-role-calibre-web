...
#
# Tags look like `v<Calibre-Web version>-<release>`:
#
# - if defaults/main.yml points at a Calibre-Web version that has never been
#   released, the release counter restarts at 0 (`v0.6.27-0`)
# - otherwise the counter is incremented (`v0.6.27-1`), but only if something
...
#
# It also avoids a failure mode the previous, commit-message-driven workflow
# ran into: squashing a Renovate pull request appends its number to the
# subject, so "Update linuxserver/calibre-web Docker tag to v0.6.27 (#7)"
# yielded the tag name `v0.6.27 (#7)-0`, which git refused outright.
...
# any of the variables derived from it (`calibre_web_container_image_tag`) can
# be picked up instead.
version="$(sed -nE 's|^calibre_web_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Calibre-Web version from $defaults_path"
	exit 1
fi

# The version value carries no leading `v` (e.g. `0.6.27`), while the tags
